function MultiInstrument_CrossWavelet_explicit()
%==========================================================================
% MULTIINSTRUMENT_CROSSWAVELET_EXPLICIT
%
% This function processes multiple GOES satellite instruments (e.g. IR and VIS)
% by computing single-frame wavelet transforms, cross-instrument cross-wavelets
% (for the same time stamp), and cross-temporal cross-wavelets (for consecutive
% frames for the same instrument). The program is structured so that no more than
% two rounds of wavelet transforms are held in memory at any time.
%
...
%% 1) VARIABLES SETUP
%----------- DATE/TIME SETTINGS ---------------------------
startDate = datetime(2023,09,28,14,0,0);      % Start of processing period
endDate   = datetime(2023,09,28,15,0,0);       % End of processing period
...
%----------- INSTRUMENT SETTINGS ----------------------------
instrumentList = {'IR','VIS'};  % List of instruments to process
...
IR_methodName = 'highpass_50_sqrt';  % Preprocessing method for IR
VIS_methodName = 'none';              % Preprocessing method for VIS
...
numContourLevels = 5;       % Number of contour levels in overlays

%----------- IMAGE ANNOTATIONS & OUTPUT ---------------------
clevfactor = 1;             % Factor for adjusting contour levels
saverose = 1;               % Flag to save the wave-rose image

%% 2) BASIC SETUP
% For each instrument, raw data is assumed to be in:
%    <rootSepacDir>\INSTRUMENT\Data
% and the single-frame wavelet results will be stored in:
%    <rootSepacDir>\INSTRUMENT\Wavelet_Results

% Retrieve file names, timestamps, and variable names for each instrument.
allInstrumentData = struct();
for iInst = 1:numel(instrumentList)
    instr = instrumentList{iInst};
    dataDir = fullfile(rootSepacDir, upper(instr), 'Data');
...
% Compute common timestamps across instruments.
...
if isempty(commonTimes)
    fprintf('No common timestamps found among instruments. Exiting.\n');
    return;
end

% Build indexing arrays to map commonTimes back to file indices.
indexArrays = struct();
for iInst = 1:numel(instrumentList)
    instr = instrumentList{iInst};
    iTimes = allInstrumentData.(instr).Times;
    [~, idxIn] = intersect(iTimes, commonTimes);
    indexArrays.(instr) = idxIn;
end

%% 3) MAIN PROCESSING LOOP
% The loop below iterates over each common timestamp (frame) and computes:
%   (a) Single-frame wavelet transforms (with preprocessing) for each instrument.
%   (b) Cross-instrument cross-wavelets (same timestamp).
%   (c) Cross-temporal cross-wavelets (for consecutive frames for each instrument).
% Memory is restricted so that only the current and previous frame's wavelet
% transforms (per instrument) are held in memory.
numCommon = numel(commonTimes);
fprintf('Found %d common frames among instruments: %s\n', numCommon, strjoin(instrumentList, ','));

% Structure to hold the previous frame's wavelet transform for each instrument.
prevWaveletSpecs = struct();
...
for f_idx = 1:numCommon
    ...
    % Temporary structure to store current frame wavelet transforms.
    waveletSpecs = struct();
    
    % Loop over instruments (single-frame processing).
    for iInst = 1:numel(instrumentList)
        instr = instrumentList{iInst};
        idx = indexArrays.(instr)(f_idx);
        thisFileName = allInstrumentData.(instr).Files{idx};
        thisFullPath = fullfile(rootSepacDir, upper(instr), 'Data', thisFileName);
        varName = allInstrumentData.(instr).VarName;
        
        % Define the output folder for single-frame wavelet results.
        singleOutDir = fullfile(rootSepacDir, upper(instr), 'Wavelet_Results', sprintf('Frame_%s', frameDateStr));
        ...
        elseif strcmpi(instr, 'VIS')
            methodName = VIS_methodName;
        else
            error('Unknown instrument type: %s', instr);
        end
        % Preprocess the data.
        data_pre = preprocessFrame(data, instr, methodName, ...
        ...
        % Build ROI squares (using the first instrument's dimensions).
        if iInst == 1
            ...
            squaresInfo = squares;  % Used for cross-instrument averaging.
        end
        ...
        % Save the single-frame netCDF if it does not already exist.
        if ~isfile(singleNcFile)
            ...
            produceAnnotatedImages(instr, spec_full, data_filt, squares, Scales, Angles, singleOutDir, frameDateStr, pixel_size_km, clevfactor, saverose, nAngles_fineFactor, nScales_fineFactor, peakDetectionFactor, numContourLevels);
        else
            fprintf('NetCDF for %s at %s already exists. Skipping creation.\n', instr, frameDateStr);
        end
        
        % Store current frame’s wavelet transform.
        waveletSpecs.(instr) = spec_full;
    end  % End instrument loop for current frame.
    
    %% 3a) CROSS-INSTRUMENT WAVELET COMPUTATION (SAME FRAME)
    if numel(instrumentList) >= 2
        ...
        for p = 1:size(allPairs,1)
            ...
            pairName = upper(strjoin(sortedPair, '_'));
            
            crossOutDir = fullfile(rootSepacDir, pairName, 'CrossWavelet_Results', sprintf('Frame_%s', frameDateStr));
            if ~exist(crossOutDir, 'dir')
                mkdir(crossOutDir);
            end
            crossNcFile = fullfile(crossOutDir, sprintf('CrossWavelet_%s.nc', frameDateStr));
            if isfile(crossNcFile)
                fprintf('Cross-wavelet netCDF for pair %s at %s exists. Skipping.\n', pairName, frameDateStr);
                continue;
            end
            
            specA = waveletSpecs.(instA);
            ...
            for iy = 1:num_squares_y
                for ix = 1:num_squares_x
                    ...
                    y_start = floor((iy - 1) * adjusted_frame_height / num_squares_y) + 1;
                    y_end = floor(iy * adjusted_frame_height / num_squares_y);
                    square_region = specA(y_buffer_range(y_start:y_end), x_buffer_range(x_start:x_end), :, :);
    ...
    for iInst = 1:numel(instrumentList)
        instr = instrumentList{iInst};
        if isfield(prevWaveletSpecs, instr) && ~isempty(prevWaveletSpecs.(instr))
            currentSpec = waveletSpecs.(instr);
            prevSpec = prevWaveletSpecs.(instr);
            crossSpec_temp = prevSpec .* conj(currentSpec);
            ...
            crossOutDir_temp = fullfile(rootSepacDir, upper(instr), 'Cross_Wavelet_Results');
            if ~exist(crossOutDir_temp, 'dir')
                mkdir(crossOutDir_temp);
            end
            crossNcFile_temp = fullfile(crossOutDir_temp, sprintf('XW_%s_%s.nc', upper(instr), frameDateStr));
            if isfile(crossNcFile_temp)
                fprintf('Cross-temporal netCDF for %s at %s exists. Skipping.\n', upper(instr), frameDateStr);
            else
                ...
                fprintf('Saved cross-temporal wavelet for %s between previous and current frame at %s -> %s\n', upper(instr), frameDateStr, crossNcFile_temp);
            end
        end
        % Update the previous frame's transform for this instrument.
        prevWaveletSpecs.(instr) = waveletSpecs.(instr);
    end
    
    % Clear temporary storage so that only the previous round remains.
    clear waveletSpecs;
end

fprintf('\nAll done. Single, cross-instrument, and cross-temporal wavelet computations complete.\n');
end
...
function data_preprocessed = preprocessFrame(data, dataType, methodName, ...
    ...
    switch upper(dataType)
        case 'IR'
            ...
            data_preprocessed = processDataMethod(data, methodName, clipMinHP, clipMaxHP, lpWidth20, lpWidth50, lpWidth100);
...
function produceAnnotatedImages(dataType, spec_full, data_background, squares, ...
    Scales, Angles, outDir, frameDateStr, pixel_size_km, clevfactor, saverose, ...
    nAngles_fineFactor, nScales_fineFactor, peakDetectionFactor, numContourLevels)
% PRODUCEANNOTATEDIMAGES
%  Creates annotated images for wavelet analysis visualization.
%
% Inputs:
%   dataType         - 'IR' or 'VIS'
%   spec_full        - 4D wavelet coefficients [Ny, Nx, nScales, nAngles]
%   data_background  - Original 2D background data
%   squares          - ROI information structure array
%   Scales, Angles   - Wavelet scales and angles
%   outDir           - Output directory for images
%   frameDateStr     - Timestamp string for the frame
%   pixel_size_km    - Pixel size in km (used for scale annotation)
%   clevfactor       - Contour level adjustment factor
%   saverose         - Flag to save the rose image (1 to save)
%   nAngles_fineFactor, nScales_fineFactor - Refinement factors for interpolation in the polar plot
%   peakDetectionFactor - Factor to compute the threshold for peak detection
%   numContourLevels - Number of contour levels for overlays
    ...
    for pk = 1:numPeaks
        ...
        text(ax1, x_peak, y_peak, sprintf('%d', pk), 'Color','k','FontWeight','bold', 'HorizontalAlignment','center','VerticalAlignment','middle');
    end
    
    % Radial and angular grid overlays.
    for i = 1:length(Scales)
        ...
        % Annotate using the pixel size in km.
        wavelength_km = Scales(i) * pixel_size_km * pi/sqrt(2);
        text(ax1, Scales(i)*1.05, 0, sprintf('%.1f km', wavelength_km), 'HorizontalAlignment','left', 'FontSize',8);
    end
    ...
    for i = 1:length(angle_ticks)
        ...
        if angle_ticks(i) <= pi
            text(ax1, x_label*1.05, y_label*1.05, angle_labels{i}, 'HorizontalAlignment','center', 'FontSize',8);
        else
            text(ax2, x_label*1.05, y_label*1.05, angle_labels{i}, 'HorizontalAlignment','center', 'FontSize',8);
        end
    end
    ...
    title(ax1, sprintf('Polar Wave-Rose: %d Significant Regions', numPeaks), 'FontSize',12, 'FontWeight','bold');
    ...
    for pk = 1:numPeaks
        ...
        peakRegions{pk} = struct('ScaleIndices', scaleIndices, 'AngleIndices', angleIndices, 'ScaleStr', scale_str{1}, 'AngleStr', angle_str{1});
    end
    ...
    for pk = 1:numPeaks
        ...
        for sq = 1:numel(squares)
            ...
            rectangle('Position',[xPos_orig, yPos_orig, w_orig, h_orig], 'EdgeColor','k','LineWidth',1);
        end
        
        titleText = {sprintf('Instrument %s - %s', dataType, frameDateStr), ...
