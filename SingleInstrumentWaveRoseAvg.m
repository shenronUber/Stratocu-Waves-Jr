...
%----------- WAVELET PARAMETERS -----------------------------
Angles = 0 : pi/7 : pi;           % Wavelet angles (in radians)
...
DisplayValuePower = 4*10^-3;
DisplayValueCoherence = 15e-05;
    ...
    if ~isempty(prevWaveletSpec)
        ...
        for iy = 1:num_squares_y
            for ix = 1:num_squares_x
                ...
                sumAbsB1 = sum(sum(abs(region_B1), 1, 'omitnan'), 2, 'omitnan');
                sumAbsB2 = sum(sum(abs(region_B2), 1, 'omitnan'), 2, 'omitnan');
                sumAbsB1 = squeeze(sumAbsB1);  % [nScales x nAngles]
                sumAbsB2 = squeeze(sumAbsB2);  % [nScales x nAngles]
                
                % Compute the phase coherence as the ratio of the new numerator to the new denominator.
                gamma_val = numerator ./ (sumAbsB1 .* sumAbsB2);
...
%     DisplayAggregatedWaveRose('Coherence_Square', coherenceMatrix, Scales, Angles, axInset, maxVal);
    ...
    nCircles = 5;
    levels_log = linspace(log10(min(Scales)), log10(max(Scales)), nCircles);
    for i = 1:length(levels_log)
        theta_ring = linspace(0, 2*pi, 180);
        [x_ring, y_ring] = pol2cart(theta_ring, levels_log(i));
...
function produceAggregatedWaveRose(labelStr, waveRoseMat, Scales, Angles, outDir, outName, saverose, maxVal)
    ...
    for i = 1:length(levels_log)
        ...
        val_linear = 10^(levels_log(i));
        text(ax1, levels_log(i), 0, sprintf('%.2f', val_linear), 'Color','k','FontSize',8,'HorizontalAlignment','left');
    end
